use crate::allosaur::utils::{AccParams, CredentialIndex, PublicKeys};
use crate::{
    accumulator::utils::Polynomial, Accumulator, Element, MembershipWitness, PublicKey, SecretKey,
};
...
pub struct Server {
    ...
    pub all_credentials: HashSet<CredentialIndex>, // \mathcal{Y}
    pub all_witnesses: HashMap<CredentialIndex, MembershipWitness>, // wits
    pub deletions: Vec<CredentialIndex>,           // list of deletions y_1,...,y_d
}
...
impl Server {
    ...
    pub fn add(&mut self, y: CredentialIndex) -> Option<MembershipWitness> {
        if self.all_witnesses.contains_key(&y) {
            return None;
        }
        // Add to set of accumulated elements
        self.all_credentials.insert(y);
    ...
    pub fn delete(&mut self, user_id: CredentialIndex) -> Option<Accumulator> {
    ...
    pub fn quick_del(&mut self, y: CredentialIndex) -> Option<Accumulator> {
        if !self.all_witnesses.contains_key(&y) {
            return None;
        }
