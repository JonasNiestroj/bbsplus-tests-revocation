use crate::{Accumulator, Element, MembershipWitness, SecretKey};
...
pub struct Credential {
    pub index: CredentialIndex,   // ID value y
...
impl Credential {
    // New "empty" user
    pub fn new(server: &Server, index: CredentialIndex) -> Credential {
        Credential {
            index,
            witness: None,
    ...
    pub fn get_index(&self) -> CredentialIndex {
        self.index
    }
    ...
    pub fn pre_update(
        ...
        // Create y, y^2, ..,. y^k-1
        let mut y_power = self.index.0.clone();
    ...
    pub fn update(&mut self, servers: &[Server], threshold: usize) -> Result<(), &'static str> {
        ...
        match self.post_update(
            self.witness.as_ref().unwrap().witness,
            threshold,
            &y_shares,
            &y_values,
            &dvs,
            ...
            Ok(new_witness) => {
                self.witness.as_mut().map(|w| w.update_witness(new_witness));
    ...
    pub fn check_witness(&self, params: &AccParams, accumulator: &Accumulator) -> bool {
        match &self.witness {
            Some(witness) => {
                Witness::ver(accumulator, &self.public_keys, params, &self.index, witness)
            }
            None => false,
        }
    }
