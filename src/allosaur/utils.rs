use crate::accumulator::{utils::Polynomial, Element, PublicKey};
use bls12_381_plus::{elliptic_curve::hash2curve::*, G1Projective, G2Projective, Scalar};
use digest::{FixedOutput, Update, XofReader};
use group::ff::Field;
use group::Curve;
...
pub type CredentialIndex = Element;
...
impl Default for AccParams {
    fn default() -> AccParams {
        ...
        let k0 = G1Projective::hash::<ExpandMsgXmd<sha3::Keccak256>>(&array, DST_G1);
        array[0] = 0xFE;
        let k1 = G1Projective::hash::<ExpandMsgXmd<sha3::Keccak256>>(&array, DST_G1);
        array[0] = 0xFD;
        let k2 = G2Projective::hash::<ExpandMsgXmd<sha3::Keccak256>>(&array, DST_G2);
        array[0] = 0xFC;
        let x1 = G1Projective::hash::<ExpandMsgXmd<sha3::Keccak256>>(&array, DST_G1);
        array[0] = 0xFB;
        let y1 = G1Projective::hash::<ExpandMsgXmd<sha3::Keccak256>>(&array, DST_G1);
        array[0] = 0xFA;
        let z1 = G1Projective::hash::<ExpandMsgXmd<sha3::Keccak256>>(&array, DST_G1);
        AccParams {
